...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      const lastEvent = await KeystrokeEvent.findOne({ userId }).sort({
        createdAt: -1,
      });

      // Calculate average inter-keystroke interval
      const events = await KeystrokeEvent.find({ userId }).select(
        "interKeystrokeInterval",
      );
      const avgInterval =
        events.length > 0
          ? events.reduce((sum, e) => sum + e.interKeystrokeInterval, 0) /
            events.length
          : 0;
